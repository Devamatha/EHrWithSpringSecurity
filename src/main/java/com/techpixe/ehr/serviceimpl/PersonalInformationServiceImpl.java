...
public class PersonalInformationServiceImpl implements PersonalInformationService {
	...
	public PersonalInformation savePersonalInformation(MultipartFile file, Long user_Id) throws Exception {
		HR user = userRepository.findById(user_Id)
		...
		if (subscriptions != null) {
			SubscriptionPlan latestSubscription = subscriptionPlanRepository.findById(subscriptions)
					.orElseThrow(() -> new RuntimeException(subscriptions + " Subscription plan not found"));
			System.out.println(latestSubscription.getPlanType() + " &&" + latestSubscription.getTotalResumes());
			try {
				if ("LifeTime".equals(latestSubscription.getPlanType())
						&& latestSubscription.getTotalResumes() <= TotalResumesCountcount) {

					throw new SizeLimitExceededException("You have reached the maximum limit of resumes",
							latestSubscription.getTotalResumes(), TotalResumesCountcount);
