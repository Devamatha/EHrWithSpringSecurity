...
public class UserServiceImpl implements UserService {
	...
	@Override
	public List<Map<String, Object>> getEmployeesByUserId(Long userId) {
		HR user = userRepository.findById(userId).orElseThrow(() -> new RuntimeException("User not found"));
		List<EmployeeTable> employeeData = user.getEmployeeTables();
		List<Map<String, Object>> response = new ArrayList<>();
		for (EmployeeTable extractdata : employeeData) {
			Map<String, Object> adddata = new HashMap<>();
			adddata.put("id", extractdata.getId());
			adddata.put("fullName", extractdata.getClients().getFullName());
			adddata.put("idNumber", extractdata.getIdNumber());
			response.add(adddata);
		}
		user.getEmployeeTables().get(0).getClients().getFullName();
